...
luck by construction, cutting outcome variance to roughly a third of a naive
game. A seven-level ``mode ladder'' adds one social channel at a time, and the
benchmark is deterministic given a seed, needs no judges or human labels, and
replays byte-identically from its logs. Running it surfaces lessons for interactive
evaluation generally. A fully afforded social layer went almost entirely
unused---first because a schema collision in our own harness recorded
off-schema replies as deliberate silence; then, once that was fixed,
because models coherently \emph{declined} to speak, treating the channel as
leakage risk; yet under one directive they used it fluently on every
opportunity. A fourth condition, run by accident when a branch merge
rewrote one phase's prompt beneath a pinned system message, proved the most
informative: a prompt that \emph{argues} for the private channels rather
than compelling speech raised adoption to 62\%, and unlike the directive
moved 55\% of that traffic onto the covert channels the benchmark exists to
price. Affordance, propensity, and capability are three different things
that an outcome-only benchmark reports as one number; elicitation
determines not only how much agents communicate but through which channel,
so ``messages sent'' ranks these conditions backwards; and an experimental
condition is a whole prompt surface, no more reproducible than its
least-pinned literal.
\end{abstract}
...
built it to answer that and learned something we consider more useful: with
the channels fully open, the models did not use them---and why, and what
eventually changed it, were recoverable only because the environment
records refusals and rendered prompts, not just actions. The design
contributes three evaluation mechanisms; running it contributes two lessons
about what such mechanisms can and cannot see.
...
\emph{chips} to a channel by differencing across modes proves harder than
we expected, for reasons \S\ref{sec:ladder} makes precise.

\textbf{4. Separating measurement failure, propensity, and capability.}
This one emerged from running the benchmark rather than designing it. Our
first mode-6 runs recorded one message across 11 games, the cause a schema
collision in our own harness: asked for a message, models replied with the
betting schema, and a missing \texttt{message} key parsed as chosen
silence---the evaluator's design error recorded as a finding about the
agents (\S\ref{sec:nondecision}). With that fixed, models emit the correct
schema and still decline to speak, giving coherent private reasons---yet a
single directive elicits fluent, purposeful communication on every
opportunity. An outcome-only benchmark reports all three situations
identically; separating them requires logging the declined option as an
event and varying elicitation while holding the environment fixed
(\S\ref{sec:elicitation}). A fourth arm sharpens it further: prompting that
\emph{argues} for the covert channels, rather than requiring speech, is the
only condition in which models used them at all, so elicitation selects the
\emph{kind} of social behaviour and not merely its volume---and the metric
most benchmarks would report, messages sent, orders those two conditions
backwards.

\textbf{5. A prompt surface is the experimental condition.} That fourth arm
was an accident: a branch merge rewrote one phase's instruction block while
the pin we relied on covered only the system message, and six of eight
seeds silently changed condition without failing a test, logging a fault,
or producing a visibly anomalous table (\S\ref{sec:promptsurface}). We
report it as a result rather than a correction because the general form
applies to every multi-phase agent harness---each phase is an
independently editable prompt literal, and a pin on a subset of them is not
a pin---and because what caught it was an artifact policy rather than a
code review: storing the fully rendered prompt with every provider call is
what made the two conditions distinguishable after the fact.

We benchmark Gemma-4-31B and Mistral Medium against a random-bot control
across modes and seeds with bootstrap intervals, Gemini~3.1 Flash Lite
additionally in the elicitation pilots, and validate the instrument with
bots-only tournaments and determinism tests.
...
design laundered into one. We were briefly prepared to report the latter as
the former (\S\ref{sec:elicitation}). We therefore state a general rule for
multi-phase agent evaluation:
...
seat per hand, so a single duplicate cycle exceeds the daily allowance of
every free tier we tested except two. Trajectory-level social evaluation is
expensive in a way single-turn benchmarks are not, and the cost falls on
the axis---repeated trials---where evaluations can least afford to
economize.
...
entirely unused under a prompt that does not ask for it---and that what
closes the gap also decides \emph{which} channels get used. We separate
four explanations; the fourth arrived by accident and is the one we would
...
(0/72 talk, 0/71 attention, 0/70 belief). Models answer the correct
schema---replies carry \texttt{message} and \texttt{intent} keys---and set
\texttt{message} to \texttt{null} on every single street, with consistent
private reasons: ``stay silent to avoid giving away any information'',
``keeping low profile early in the game'', ``remain quiet to observe how
the opponents react''. They are not failing to use the channel; they are
declining it, reasoning about communication almost exclusively as a
leakage risk rather than as an instrument for shaping other players.
...
affordance closes and that one directive closes completely.

\textbf{Adoption is degenerate in kind, not only in volume.} All 19
directed messages were public \texttt{speech}. Under every condition
described so far, no model sent a whisper, note, gesture, or accusation---the
covert channels that carry the strategic content, and the only ones the
perception, interception, and heat machinery can price. Compelled to
communicate, models reach for the one channel with no concealment, no
addressee, and no risk.

\textbf{(4) Elicitation selects the channel, not just the volume.} The
fourth arm was not designed; it was a mistake we caught after the fact, and
it is more informative than the arm we meant to run
(\S\ref{sec:promptsurface} explains how it happened). Its talk prompt keeps
the same schema and the same channels but argues for using them: it tells
agents that silence is ``the exception, not the default'', quotes the
published interception odds, and states that ``the private channels are
where influence turns into chips''. It answers, point for point, the three
...
rolls fired for the first time on traffic a model chose to send, shredding
23 whispers into fragments overheard by a third seat. Adoption is partial
rather than total (186 replies still chose silence explicitly), which is
what distinguishes this arm from the directed one---agents are choosing,
not complying.
...
metric requiring a target, a secret, or a risk---detection, information
control, cover, the whole interception apparatus---is zero in the first and
...
misleads. The content confirms it: whispers open private lines and keep
them open (\emph{``I hit the five.''}, intent ``build alliance with P2
while subtly intimidating P1''), while the public channel carries the
recorded lies the environment exists to price without a judge
(\emph{``That Queen is a bit of a buzzkill, isn't it?''}, intent ``downplay
the strength of my KK to induce action''). The two models also differ
sharply in propensity---Gemma-4-31B 246 messages, 61\% whispered; Mistral
Medium 59, 34\%---which no outcome-only leaderboard would reveal.
...
Neutral, phase-explicit       & 2  & 34  & 0\%    & 0\%    & 0   & --- \\
...
reporting only outcomes would score every entrant here as socially inert
and invite the conclusion that these models cannot deceive or coordinate,
when what it measured is \emph{propensity} under one prompt. Separating
propensity from capability requires logging the declined option as an
event---the road not taken has to be in the trajectory---and an elicitation
arm that varies framing while holding the environment fixed.

\subsection{A prompt condition is a surface, not a string}
\label{sec:promptsurface}

The fourth arm exists because we ran it without meaning to, and the way it
happened defeated the specific safeguard we had built against it.

Extending the headline benchmark with more seeds, we pinned the prompt
condition with the environment variable the codebase provides for exactly
this purpose---the one whose commit message records it as ``verified
...
covered the system message, and between the published runs and the
extension a branch merge had rewritten the talk-phase block from a neutral
description of the channels into an argument for using them---the advocacy
prompt of arm 4. Two of eight seeds were collected under one condition and
six under another, and the run reported one leaderboard over all eight.

Nothing failed. No test broke, no fault was logged, the schema-miss
detector of \S\ref{sec:nondecision} stayed at zero, and the resulting table
was well formed and wrong---not visibly anomalous either, since it shifted
every entrant toward the mean and widened one interval across zero, which
reads as ordinary added variance. What exposed it was neither the code nor
the summary statistics but the archived transcripts: because every provider
call stores its fully rendered prompt, the two conditions could be diffed
character by character and the drift localized to one instruction block. We
draw three conclusions, none specific to poker.
...
\textbf{We were lucky in the size of the effect.} The drift announced
itself only because the rewritten prompt moved behaviour from 0 messages to
305. A revision shifting adoption by a few percent, or changing only tone,
would have produced a table just as well formed, just as wrong, and with
nothing conspicuous to investigate---which is why the literature's silence
on prompt-surface drift is more likely to mean it is invisible than that it
is rare. An evaluation result is a claim about a prompt surface, no more
reproducible than the least-pinned literal in it.

\subsection{Main results: mode 6}
\label{sec:main}

\begin{table}[tbp]
\caption{Mode-6 leaderboard under the neutral prompt surface, seeds
$\{41,42\}$, 8 hands. Intervals are bootstrap 95\%, indicative only at this
seed count; ``Rep./Flt.'' counts repairs and parse faults. Detection,
information control, and cover are omitted: with no covert messages every
entrant ties by construction.}
...
Two details temper the headline. The entrant with the most chips is not the
most disciplined---Mistral's play required illegal-action repairs in every
seed while Gemma required none, so a benchmark pricing protocol compliance
alongside outcome would rank them differently. And, more important for
interpreting this table, \emph{the mode-6 social layer contributed nothing
to it}: entrants sent zero messages in every rotation of every seed
(\S\ref{sec:elicitation}), so detection, information control, and cover
have no data behind them. What Table~\ref{tab:leaderboard} measures is
...
Modes 0 and 6 present the same entrants with the same seeded deals, so a
model extracting value from the social channels should move between them.
It does not, in an instructive way: on seed 41 the modes are the same game
to within one adjusted chip, while on seed 42 the comparison swings 434
chips and reorders the table---with zero messages sent in either mode, so
no channel can be responsible. \textbf{Pairing on identical decks cancels
environment stochasticity; it does nothing about agent stochasticity.}
Isolating a condition effect over stochastic agents needs paired agent
sampling---fixed decoding seeds, which production APIs do not reliably
expose---or many more trials than a free-tier budget allows
(Appendix~\ref{app:ladder}).

\subsection{What elicited talk actually contains}
\label{sec:case}

Storing declared intent beside the surface form lets us inspect the content
of communication and not merely its count, and the two elicited arms differ
in kind. Directed talk is pleasantry over instrumental intent---\emph{``Good
luck everyone!''} to ``encourage limpers to keep the pot small with my weak
hand''---and never acquires a target, a secret, or a risk. Advocacy talk
does: it opens private lines, trades intel, and proposes coalitions, which
is what finally exercised the perception machinery of
\S\ref{sec:perception} by a model's own choice rather than by the test
suite. The intent channel also catches the directed arm reporting on
itself. One intent reads, in full, ``establish a friendly table image
\emph{and satisfy the communication requirement}''---the model naming the
instruction as part of its own motivation, which is the clearest evidence
that the directed condition is a reachability control rather than a
behavioural one, and an argument for recording declared intent in any
elicitation study: it is the channel in which an agent will tell you it is
...
price of judge-free ground truth, and also the point: the benchmark
measures what agents \emph{do} with partial observations. Agents could
likewise lie about their intents, but no headline metric depends on intent
truthfulness, and a systematic intent--behaviour mismatch is itself
measurable.
...
what happened. The ladder was our control, and \S\ref{sec:ladder} shows
...
the most serious limitation. Two models of moderate size declined the
social channels and a third behaved identically in pilots, but we cannot
conclude that frontier models would, and there are reasons to doubt
it---long-horizon planning is exactly the capability that makes a hand-1
codebook worth setting up for a hand-4 payoff. What we can say is stronger
than it may appear: the declination is not a capability ceiling, since the
same models communicate competently when directed; not a parsing artifact,
since schema misses are zero; and not noise, since it holds across every
rotation of every seed, under both the neutral and the socially framed
prompt, and across three model families. It is, however, prompt-fragile in
a specific direction, and we would rather bound the claim than have a
reader do it: declination survives a paragraph asserting that social play
matters and does not survive a talk prompt that rebuts the reasons for
silence. The honest statement is therefore about a surface and not about
the models---these models decline these channels under a prompt that does
not advocate for them---which is the shape of claim
\S\ref{sec:promptsurface} argues evaluation results should be forced to
take. A preliminary probe suggests the pattern persists at scale (two much
larger models, 550B and 120B, produced five well-formed talk decisions
between them, all declining), but five decisions is not a result; running
it at frontier scale is the first experiment we would fund.

Bootstrap intervals over a handful of seeds are likewise indicative, not
decisive, and are reported to convey uncertainty rather than to claim
significance we have not earned; deals are generated procedurally, so
adding seeds is purely a matter of budget.
...
game: every participant is a model, no human is deceived, the only currency
is chips in a sandbox, and the benchmark returns no gradient---we measure
...
issues. \emph{(ii) Non-decision instrumentation}: multi-phase protocols are
the norm in agent harnesses, and in each an off-schema reply can masquerade
as a deliberate no-op, which the parsing rule of \S\ref{sec:nondecision}
prevents in a few lines. \emph{(iii) Paired-trajectory scoring}: where an
evaluation can replay the \emph{identical} stochastic context for every
system under test---same seeded environment, same simulated user, same tool
failures---differencing against the per-context mean removes context
difficulty and lets small trial budgets produce usable intervals.
...
beside every call---the cheapest of the four, and the one whose absence
silently invalidated six of our own runs.

\subsection*{Conclusion}

\textsc{BluffHouse} measures deception, detection, and information control
without a judge in the loop, but the part we did not design is the part
worth keeping: a social layer can be fully afforded, fully instrumented,
and fully unused, and what closes that gap decides which channels get used,
so an elicitation study measures a prompt surface rather than a model---and
that surface is plural. Log the declined option, log the rendered prompt,
and pin every literal you claim to hold fixed.
...
source of truth; per-agent observations, all scoring, and the replay viewer
are deterministic, RNG-free projections of it. Randomness enters in exactly
two places---the deck and the perception rolls---both drawn from
SHA-256-namespaced sub-seeds of the run seed, so given a seed and fixed
agents a run is byte-identical, a property tested for every mode. Events an
agent failed to notice are \emph{omitted} from its observation stream: real
...
the environment. The gap between them is how deception stays measurable
without labels: the agent tells the environment what it is really doing
while showing the table something else. The environment records this social
truth but never referees it---a lie that lands is the benchmark working,
not a rules violation---and \S\ref{sec:limits} discusses why the metrics do
not depend on agents being honest in the intent channel.
...
probability $p = b(\text{modality}) \times (1 - \text{subtlety}) \times
(1 - \text{stealth}) \times m_{\text{att}} \times (1 - \text{noise})$ and
one seeded roll. Base rates $b$ run from $0.35$ for a whisper down to
$0.18$ for eye contact, and the attention multiplier $m_{\text{att}}$ is
described below (Appendix~\ref{app:formulas}). \emph{Subtlety} is the
sender's own dial and cuts both ways: it suppresses interception \emph{and}
raises the chance the intended recipient misses the message; noise is
raised by staged distractions (mode~6). Outcomes are recorded inside the
event as ground truth
(Figure~\ref{fig:receptions}, Appendix~\ref{app:schema}): the target got it
...
Malformed play never crashes a game: prose replies get one correction round
trip then a safe fallback, illegal actions are coerced to the closest legal
move and logged as repairs, and illegal communications are dropped rather
than downgraded to a more public channel, so a privacy failure cannot leak
content (Appendix~\ref{app:schema}).
...
prompt revision, and we pin it deliberately. The condition used throughout
is the neutral \emph{surface} reproduced in \S\ref{app:prompts}: a system
message plus four phase instruction blocks that state the rules, the
schemas, and the objective, and say nothing about whether communicating is
worthwhile. The elicitation arms (\S\ref{sec:elicitation}) differ from it
only by the paragraphs quoted there.

We say ``surface'' rather than ``prompt'' because the distinction cost us
six seeds (\S\ref{sec:promptsurface}). One environment variable now selects
every literal in the published condition at once---system message and all
four phase blocks---and a regression test asserts each is byte-identical to
the archived text, so the condition cannot drift one literal at a time. The
generalization we would press on anyone building a multi-phase harness: the
number of prompt literals your experiment depends on is at least the number
of phases it has, and a pin that covers a proper subset of them is not a
pin.
